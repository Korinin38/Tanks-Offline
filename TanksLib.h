...
//!
//! @file    TanksLib.h
//! @brief   Ѕиблиотека функций проекта "Tanks Offline"
//!
//!          $Version: 0.5.9$
//!          $Date: 2018-03-22 20:42:74 +0500 $
//!
//!          Ѕиблиотека TanksLibrary содержит в себе функции, необходимые дл€ работы проекта.
//!          ¬ проекте используетс€ библиотека TXLibrary, созданна€ »льЄй ƒединским
//!
//! @warning <b>Ёто бета-верси€. ƒл€ использовани€ требуетс€ согласование с автором библиотеки.</b> @nn
//!
//!
//-----------------------------------------------------------------------------------------------------------------
//!
//! @defgroup Drawing   –исование
//! @defgroup Interface »нтерфейс
//! @defgroup Logic     Ћогика
//}
//=================================================================================================================
...
/*!
    @ingroup Drawing
    @brief   ќтрисовка карты
             –исует карту, основыва€сь на структуре mapMas[][]
...
/*!
    @ingroup Drawing
    @brief   –исование экрана приветстви€
...
/*!
    @ingroup Drawing
    @brief   –исование экрана инициализации карты
    @param   xWindowSize размер окна по X
    @param   yWindowSize размер окна по Y
    @usage @code
             drawMapInit(800, 600);
...
/*!
    @ingroup Drawing
    @brief   –исует предупреждение, по€вл€ющеес€ при нахождении файла сохранени€ карты
    @param   xWindowSize размер окна по X
    @param   yWindowSize размер окна по Y
    @warning ≈сли в процессе игры выводитс€ данный экран, значит, у ¬ас есть файл сохранени€ в папке игры;
             „тобы загрузить его, необходимо нажать Y; „тобы проигнорировать его, необходимо нажать N
...
/*!
    @ingroup Drawing
    @brief   –исование экрана загрузки карты
    @param   xWindowSize размер окна по X
    @param   yWindowSize размер окна по Y
    @usage @code
             drawMapLoad(800, 600);
...
/*!
    @ingroup Drawing
    @brief   –исует танк из альфа-версии
    @param   t         ќбъект структуры tank
    @param   xOfCenter X-координата центра карты
    @param   yOfCenter Y-координата центра карты
    @param   mapSize   –азмер карты
...
/*!
    @ingroup Drawing
    @brief   —оздаЄт окно выбора разрешени€ экрана
...
/*!
    @ingroup Drawing
    @brief   ѕодсвечивает клетки в которые танк с текущей скоростью может попасть в данный ход
...
/*!
    @ingroup Drawing
    @brief   –исует кнопку отвечающую за вкачивание перков у танков
...
/*!
    @ingroup Drawing
    @brief   –исует кнопку отвечающую за выход из игры
    @param   xCenter            координата по x середины кнопки
    @param   yCenter            координата по y середины кнопки
    @param   radius             радиус кнопки
    @param   fillColor          цвет заполнени€ внешней части кнопки
...
/*!
    @ingroup Drawing
    @brief   –исует кнопку отвечающую за вход в режим редактора карт
    @param   xCenter             координата по x середины кнопки
    @param   yCenter             координата по y середины кнопки
    @param   Color               цвет заполнени€ кнопки
...
/*!
    @ingroup Logic
    @brief   »нициализаци€ карты
             ѕри наличии сохранЄнной карты выгружает еЄ из файла, иначе генерирует случайную карту (несв€зную)
    @param   sav       ѕри sav=1 выгружает из файла, иначе генерирует случайную
...
/*!
    @ingroup Logic
    @brief   —оздание графа на основе карты из структуры mapMas[][] и проверка его на св€зность (с возможным последующим исправлением)
    @param   onlycheck  ѕри значении 1 только провер€ет на св€зность, иначе исправл€ет
    @param   mapDat1    оличество строк
    @param   mapDat2    оличество столбцов
    @return  1, если карта €вл€етс€ с€зной, и 0 в противном случае
...
/*!
    @ingroup Logic
    @brief   ѕровер€ет данную точку графа из структуры graph[][5] на св€зность с точкой (0, 0)
             –екурсивный алгоритм, необходимый дл€ работы функции <b>correctMapChecker()</b>
    @param   n  номер
    @return  1, если точка €вл€етс€ св€зной, и 0 в противном случае
...
/*!
    ...
    @see     mapMas[][], graph[][5], correctMapChecker()
    @usage @code
             mapSavedChecker(); //Ќу да, всЄ так просто
    @endcode
*/
//}----------------------------------------------------------------------------------------------------------------
...
/*!
    @ingroup Logic
    @brief   ќсуществл€ет контроль над камерой
             Ѕлокирует возможность камеры улететь далеко-далеко от карты и потом не найти еЄ
             Ѕлокировка осуществл€етс€ во избежание инцидентов и отрицательных отзывов. —пасибо за понимание!
    ...
    @usage @code
             ...
             {
                 yOfCenter++;
                 mapBoundController(&xOfCenter, &yOfCenter, 6, 6, 10, xWindowSize, yWindowSize); //теперь камера остановитс€ воврем€, и мы будем видеть еЄ всегда
             }
    @endcode
*/
//}----------------------------------------------------------------------------------------------------------------
...
/*!
    @ingroup Logic
    @brief   ѕровер€ет возможность перемещени€ танка от указанной клетки с текущей скоростью
             –екурсивный алгоритм, вызывает функцию drawTankMovementGlow(), вызываетс€ функцией interfaceTankMoveCheck()
    ...
    @param   spd       текуща€ скорость танка
    @param   turn      1, если мы провер€ем танк, который ходит сейчас; 0 в противном случае
    @param   xOfCenter X-координата центра карты
    @param   yOfCenter Y-координата центра карты
    @param   mapDat1    оличество строк
    @param   mapDat2    оличество столбцов
    @param   mapSize   –азмер карты
    @param   a         ѕо умолчанию 1 - игнорирование танка в начальной клетке; измен€ть на 0 <i> не рекомендуетс€</i>
...
/*!
    @ingroup Logic
    @brief   ‘ункци€ перевода строки типа string в массив символов типа char*

    @param   s      переводима€ строка
...
/*!
    @ingroup Logic
    @brief   ‘ункци€ перевода значений любого числового типа в строку string
...
/*!
    @ingroup Logic
    @brief   ќчистка следов после chooseStart()
             автоматически вызываетс€ в самом chooseStart()
...
/*!
    @ingroup Logic
    @brief   ѕодготовка мусора в массиве mapMas[][] дл€ работы chooseStart()
             автоматически вызываетс€ в самом chooseStart()
