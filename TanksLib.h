...
/*!
    ...
    @warning <b>Внимание!</b>  k= <i>радиус</i> танка, <b>не</b> <i>диаметр</i> (<big>у</big> <ins>автора<ins> <del>проблемы</del> <sub>с</sub> <code>тегами</code> <dfn>шрифта<dfn> <b>HTML<b>)
...
/*!
    ...
    @brief   Рисование экрана параметров всех танков
             Вызывает функции пиктограмм, такие как drawHP(), drawAttack(), etc.
    ...
    @usage @code
             int tankAmount = 2;
             tank t[tankAmount];
             for (i=0;i<tankAmount;i++) //объявление всех параметров танков, смотрите структуру tank
             {
                ...
             }
...
/*!
    ...
    @param   timeTurnChange     некоторое время (текущее, на данный момент), когда кнопка, отвечающая за смену хода, заблокированна(чтобы избежать случайных окончаний хода)
    @param   xWindowSize        размер окна по X
    @param   yWindowSize        размер окна по Y
    @usage @code
             int ttc=40;
...
/*!
    ...
    @param   crossColor         цвет заполнения крестика (ну ладно, плюсика) кнопки
...
/*!
    @ingroup Drawing
    @brief   Рисует кнопку "Начать игру"
             является по совместительству кнопкой "Продолжить игру", если был произведён выход в меню
    @param   xCenter             координата по x середины кнопки
    @param   yCenter             координата по y середины кнопки
    @param   Color               цвет заполнения кнопки
    @param   notStartButContinue При значении 1 рисует кнопку "Продолжить игру", в противном случае кнопку "Начать игру"
    @usage @code
...
/*!
    ...
    @param   yCenter             координата по y середины кнопки
    @param   Color               цвет заполнения кнопки
    @param   comingSoon          при значении 1 снизу подписывает юзверю, что она НЕ РАБОТАЕТ ХВАТИТ ТЫКАТЬ НИЧЕГО НЕ ПРОИЗОЙДЁТ
    @usage @code
...
/*!
    @ingroup Drawing
    @brief   Рисует сердце - картинку для обозначения HP танка
             Функция-"пиктограмма"
...
/*!
    @ingroup Drawing
    @brief   Рисует скрещенные мечи - картинку для обозначения урона выстрелов танка
             Функция-"пиктограмма"
    @param   x             координата по x середины сердца
    @param   y             координата по y середины сердца
    @param   k             размер сердца

    @warning Да, мы знаем, что танкам не нужен меч
             он делает больно иначе <sup>надо было уйти после девятого</sup>
...
/*!
    ...
    @param   distanceOfStart  минимальное расстояние, на котором могут быть расставлены танки
    @param   xOfCenter        X-координата центра карты
    @param   yOfCenter        Y-координата центра карты
    @param   mapSize          Размер карты
    @param   mapDat1          Количество строк
...
/*!
    ...
    @param   t                структура tank (конкретно - танк, в который стреляли)
    @param   xOfCenter        X-координата центра карты
    @param   yOfCenter        Y-координата центра карты
    @param   mapSize          Размер карты
    @param   mapDat1          Количество строк
    @param   mapDat2          Количество столбцов
    @param   boomCd           стадия анимации
...
/*!
    ...
    @param   turn      1, если мы проверяем танк, который ходит сейчас; 0 в противном случае
    @param   xOfCenter X-координата центра карты
    @param   yOfCenter Y-координата центра карты
    @param   mapDat1   Количество строк
    @param   mapDat2   Количество столбцов
...
/*!
    @ingroup Logic
    @brief   Очистка следов после chooseStart()
             автоматически вызывается в самом chooseStart()
...
/*!
    @ingroup Logic
    @brief   Подготовка мусора в массиве mapMas[][] для работы chooseStart()
             автоматически вызывается в самом chooseStart()
...
/*!
    @ingroup Logic
    @brief   Отвечает за файл pcPref.txt, в который подгружается разрешение окна
             Вызывает функцию DrawDefinitionChoose()
...
/*!
    @ingroup Interface
    @brief  Отвечает за передвижение карты и её размер
    @param  *xOfCenter   указатель на X-координату центра карты
    @param  *yOfCenter   указатель на Y-координату центра карты
...
/*!
    @ingroup Interface
    @brief   Функция, отвечающая за подсвечивание возможных ходов и атаку при нажатии на танк
    ...
    @usage @code
                        //вряд ли это вам понадобится, так что...
                        //я просто вырежу часть своего кода и вставлю сюда
             ...        //здесь должна быть инициализация массива t[] и переменных xOfCenter, yOfCenter, mapSize, mapDat1, mapDat2, tankAmount, timeMouseTankIgnore, timeMouseTankAttackIgnore, turn, boomed
...
/*!
    ...
    @param   *boomCd    указатель на игнор нажатия атаки
    @param   *boomed    указатель на номер атакованного танка

    @usage @code
             ...        //здесь должна быть инициализация массива t[] и переменных tankAmount, xOfCenter, yOfCenter, mapSize, mapDat1, mapDat2, tankNumber, timeMouseTankAttackIgnore, boomed
...
/*!
    @ingroup Interface
    @brief   Меняет значения разрешения игрового окна
             Вызывается в побочном окне, созданном функцией drawDefinitionChoose()
...
/*!
    @ingroup Interface
    @brief   Отвечает за передвижение танка
    ...
    @warning На данный момент реализован лишь ход на одну клетку. Более дальние дистанции преодолевать не позволяет стрела в колене. Конец приключениям!
...
/*!
    @ingroup Interface
    @brief   Отвечает за работу кнопки смены хода

    @param   xWindowSize                размер окна по x
    @param   yWindowSize                размер окна по y
    @param   *timeMouseNewTurnIgnore    указатель на игнор повторного нажатия кнопки: положительное значение переменной отключает работу функии - работает только при значении 0
...
/*!
    @ingroup Interface
    @brief   Функция, отвечающая за кнопку выхода в меню
    @param   xWindowSize    размер окна по x
    @param   yWindowSize    размер окна по y
...
/*!
    ...
    @param   tankAmount                     количество танков
    @param   tankNumber                     номер танка
    @param   *mainButtonClicked             триггер нажатия главной кнопки (кнопки распределения перков)
...
/*!
    @ingroup Interface
    @brief   Универсальный алгоритм проверки наведения на круглые кнопки
...
/*!
    ...
    @return  0, если нажата кнопка "Выход", 1 в противном случае
...
/*!
    @ingroup Interface
    @brief   Функция, отвечающая за расстановку танков на игровое поле (в начале игры и после воскрешения)
    ...
    void chooseStartInitialise(int distanceOfStart, int mapDat1, int mapDat2)
    {
        for (int i = 0; i < mapDat1; i ++)
        {
            for (int j = 0; j < mapDat2; j ++)
            {
                ...
                {
                    for (int i1 = std::max(0, i-distanceOfStart); i1 <= std::min(mapDat1-1, i + distanceOfStart); i1++)
                        for (int j1 = std::max(0, j - distanceOfStart); j1 <= std::min(mapDat2-1, j + distanceOfStart); j1++)
                            if (
    ...
    bool secretFunction(int* ss)
    {
        switch(*ss)
        {
            ...
            default:
                break;
                //{
                //   Не понравилось описание, что заглянул сюда?
                //   Ну а что вы хотели? Читы просто так на дороге не валяются
                //}
        }
        return 0;
    }
